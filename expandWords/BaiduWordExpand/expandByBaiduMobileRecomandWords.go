...
func ExpandBaiduRecommendWords(word string) (keywords []string, err error) {
	rs, err := baidu.GetBaiduMobileResultsByKeyword(word, 1)
	if err != nil {
		return
	}
	if len(*rs) == 0 {
	...
	wecon, err := httpUtil.GetWebConFromUrlWithAllArgs(RecommendWordsUrl, map[string]string{"authority": "m.baidu.com",
		"method":                    "GET",
		"scheme":                    "https",
